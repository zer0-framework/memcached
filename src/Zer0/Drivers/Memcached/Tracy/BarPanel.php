...
namespace Zer0\Drivers\Memcached\Tracy;

use Zer0\Drivers\Memcached\MemcachedDebug;

/**
 * Class BarPanel
 * @package Zer0\Drivers\Memcached\Tracy
 */
class BarPanel implements \Tracy\IBarPanel
{
     ...
     * @param MemcachedDebug $memcached
     */
    /**
     * BarPanel constructor.
     * @param MemcachedDebug $memcached
     ...
     * @param MemcachedDebug $memcached
     */
    public function __construct(MemcachedDebug $memcached)
    {
    ...
    {
        $queries = $this->memcached->getQueryLog();
        if (count($queries) === 0) {
            return '';
        }
        $html = '<img src="' . $this->icon . '" alt="Memcached" /> ';
        $count = count($queries);
        if ($count == 1) {
            $html .= '1 query';
        } else {
